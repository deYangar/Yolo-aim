...
class Predict(object):
    def __init__(self):
        ...
        parser.add_argument('--use-cuda', type=bool, default=config.getboolean('SECTION 1', "use-cuda"),
                            help='是否使用cuda')
        ...
        parser.add_argument('--show-fps', type=bool, default=config.getboolean('SECTION other', "show-fps"),
                            help='是否显示帧率')
        parser.add_argument('--show-label', type=bool, default=config.getboolean('SECTION other', "show-label"),
                            help='是否显示标签')
        ...
        parser.add_argument('--auto-fire', type=bool, default=config.getboolean('SECTION other', "auto-fire"),
                            help='自动开火；True为开启，False为关闭')
        ...
        parser.add_argument('--lock-button', type=str, default=config.get('SECTION other', "lock-button"),
                            help='lock按键；只支持鼠标按键')
        parser.add_argument('--head-first', type=bool, default=config.getboolean('SECTION other', "head-first"),
                            help='是否优先瞄头')
        ...
        self.args.model_path = cur_dir + self.args.model_path  # 模型的完整路径
        self.args.lock_tag = [str(i) for i in self.args.lock_tag]  # 将标签由int转为str
        self.args.lock_choice = [str(i) for i in self.args.lock_choice]  # 将锁定目标由int转为str

        device = 'cuda' if self.args.use_cuda else 'cpu'  # 判断用cpu还是gpu预测
        half = device != 'cpu'  # 当device为cuda时 half = cuda
        imgsz = self.args.imgsz  # 预测模型的大小

        conf_thres = self.args.conf_thres  # 置信度
        iou_thres = self.args.iou_thres  # 交并比阈值

        top_x, top_y, x, y = get_parameters()  # 获取屏幕的左上角和右下角的坐标
        len_x, len_y = int(x * self.args.region[0]), int(y * self.args.region[1])  # 检测范围的宽和高
        top_x, top_y = int(top_x + x // 2 * (1. - self.args.region[0])), int(
            top_y + y // 2 * (1. - self.args.region[1]))  # 检测范围的左上角起始坐标
        monitor = {'left': top_x, 'top': top_y, 'width': len_x, 'height': len_y}  # 检测范围的具体参数

        model = load_model(self.args)  # model由FP32转为FP16
        stride = int(model.stride.max())  # model的最大stride
        names = model.module.names if hasattr(model, 'module') else model.names  # 模型里面的标签名字

        self.lock_mode = False  # 全局变量是否开启锁定
        self.lock_button = eval('pynput.mouse.Button.' + self.args.lock_button)  # 开启锁定的键位
        ...
        if self.args.show_window:
            ...
            cv2.resizeWindow('Detect', int(len_x * self.args.resize_window), int(len_y * self.args.resize_window))
        ...
        # # 当按“END”键时退出整个程序
        # # ------------------------------------------------#
        listener1 = pynput.keyboard.Listener(on_press=self.on_press)
        listener1.start()
        ...
        while self.close:
            ...
            for i, det in enumerate(pred):
                ...
                if len(aims):
                    if self.lock_mode:
                        lock(aims, mouse, top_x, top_y, len_x, len_y, self.args)  # 锁定目标
                    ...
                    if self.args.show_window:
                        for i, det in enumerate(aims):
                            ...
                            cv2.rectangle(img0, top_left, bottom_right, (0, 255, 0), thickness=self.args.thickness)
                            if self.args.show_label:
                                cv2.putText(img0, tag, top_left, cv2.FONT_HERSHEY_SIMPLEX, 0.7, (235, 0, 0),
                                            4)  # 对标签进行显示
            ...
            if self.args.show_window:
                ...
                if self.args.show_fps:  # 显示fps值
                    cv2.putText(img0, "FPS:{:.1f}".format(1. / (time.time() - t0)), (20, 30), cv2.FONT_HERSHEY_SIMPLEX,
                                1,
                                (0, 0, 220), 2)
                ...
                if self.args.top_most:  # 是否保持实时检测窗口置顶
    ...
    def on_click(self, x, y, button, pressed):
        if button == self.lock_button:  # 如果按键为开启锁定的键位
            if self.args.hold_lock:  # 如果开启按住锁定
                if pressed:
                    self.lock_mode = True
                    print('locking...')
                else:
